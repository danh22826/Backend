...
function renderRegionList() {
  ...
  el.innerHTML = cities.map(c => `
    <button
      ...
      onclick="selectCity('${c.maThanhPho}')">
...
async function selectCity(maTP) {
  ...
  el.innerHTML = raps.map(r => `
    <button
      ...
      onclick="selectRap('${r.maRap}', '${escapeHtml(r.tenRap)}')">
...
async function selectRap(maRap, tenRap) {
  ...
  document.querySelectorAll('.mv-cinema-chip').forEach(btn => {
    const match = btn.getAttribute('onclick')?.includes(`'${maRap}'`);
...
async function loadPhimForRap() {
  ...
  el.innerHTML = sorted.map(m => {
    ...
    return `
      <button class="${cls}"
        ${hasShowtime ? `onclick="selectPhim('${m.maPhim}', '${escapeHtml(m.tenPhim)}')"` : 'disabled'}
      >
...
async function selectPhim(maPhim, tenPhim) {
  ...
  document.querySelectorAll('.mv-phim-item').forEach(btn => {
    const match = btn.getAttribute('onclick')?.includes(`'${maPhim}'`);
    btn.classList.toggle('active', !!match);
  });
  ...
  return `
    <div class="mv-st-movie-block">
      <a class="mv-st-movie-title"
         href="chi-tiet-phim.html?id=${encodeURIComponent(phim.maPhim)}">
        ${getAgeBadge('')}
